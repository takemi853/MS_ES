...
def main():
    ms_filename = f'./log/log_ms{n}_update220210.csv'
    semi_ms_filename = f'./log/log_semi_ms{n}_update220210.csv'
...
#                 for i in tqdm(range(1000000)):
                    ...
                    if fitness_all(ms) == 0:
                        ...
                        ms_list.append(ms[0])
                        count += 1
                        df_log = df_log.append({'ms': ms, 'generation': i, 'Loop_limit':loop_limit},
                                               ignore_index=True)
                        df_log.to_csv(f'{ms_filename}', index=False)
                        break
                    elif fitness_semi(ms) == 0 and flg == False:
                        ...
                        flg = True
                        if semi_ms_flg == True:
                            df_semi_log = df_semi_log.append({'ms': ms, 'generation': i, 'Loop_limit':loop_limit},
                                                             ignore_index=True)
                            df_semi_log.to_csv(
                                f'{semi_ms_filename}', index=False)
                    # Early Stopping (100回以上 同じ解をとったらストップ)
                    if fitness_semi(ms)!=0:
                        if ms == _ms:
                            # dup_count += (1 - flg_rate)
                            dup_count += 1
                            if dup_count >= 100:
    #                             ms = df_semi_log['ms'][-1:]
                        ...
                        if ms == _ms:
                            ...
                            if dup_count >= 100:
#                                 print(df_semi_log)
                                if len(df_semi_log)!=0:
                                    ms = df_semi_log.iloc[-1,1]
                    flg_count += flg_rate
                    _ms = copy.deepcopy(ms)
                print(
                    fitness_semi(ms),
