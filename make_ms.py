...
def main():
    ms_filename = f'./log_ms{n}_220215.csv'
        ...
        if semi_ms_flg == False:
            if len(ms_count) < 100:
                ...
                # for i in (range(100000)):
                    ...
                    log_fitness.append(fitness_all(ms))
                    log_semi_fitness.append(fitness_semi(ms))
                    # print(flg_rate)
                    ...
                    if fitness_all(ms) == 0:
                        ...
                        ms_list.append(ms[0])
                        count += 1
                        df_log = df_log.append({'ms': ms, 'generation': i, 'time': time.time() - start, 'fitness_all':log_fitness,'fitness_semi':log_semi_fitness},
                                               ignore_index=True)
